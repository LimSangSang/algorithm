...
'''
# 나의 답
# 없다.. 아이디어는 있었지만 코드로 적히지 않았음

# 해설
'''
주어진 N에 대하여 '최대한 많이 나누기'를 수행하면 된다. 문제에서는 N의 범위가 10만 이하이므로, 이처럼 일일이 1씩 빼도 문제를 해결할 수 있다. 하지만 N이 100억 이상의 큰 수가 되는 경우를 가정했을 때에도 빠르게 동작하려면, N이 K의 배수가 되도록 효율적으로 한 번에 빼는 방법으로도 소스코드를 작성할 수 있다.
'''
# 단순하게 푸는 예시
